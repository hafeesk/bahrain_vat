...
def execute(filters=None):
	...
	purchase_data = frappe.db.sql('''select COALESCE(sum(net_total),0),COALESCE(sum(total_taxes_and_charges),0) from `tabPurchase Invoice` si where date(posting_date) BETWEEN "{}" and "{}" and docstatus = 1'''.format(filters.get("from_date"),filters.get("to_date")))
	...
	frappe.msgprint(frappe.as_json(sales_data))
	...
	data = [[lst_description[0],sales_data[0][0],sales_data[0][1]],[lst_description[1],0.00,0.00],[lst_description[2],sales_data[0][0],sales_data[0][1]],[lst_description[3],purchase_data[0][0],purchase_data[0][1]],[lst_description[4],0.00,0.00],[lst_description[5],sales_data[0][0]+purchase_data[0][0],sales_data[0][1]+purchase_data[0][1]]]
